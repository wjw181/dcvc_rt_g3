...
class DMC(CompressionModel):
    ...
    def forward(self, x, qp,flag_I=False):
        ...
        if(flag_I):
            x_unshuffled = F.pixel_unshuffle(x, 8)
            _, _, H_feat, W_feat = x_unshuffled.shape
            B, C, H, W = x.shape
            feature_shape = (B, g_ch_d, H_feat, W_feat)
            empty_feature = torch.zeros(feature_shape, 
                                        device=x.device, 
                                        dtype=x.dtype)
            feature = self.feature_adaptor_p(empty_feature)
        else:
        ...
        mse = 2.4 * Y_mse + 1.2*U_mse + 1.2*V_mse
        ...
        bits_y = self.get_y_gaussian_bits_safe(y_for_bit, scales_hat)


        # bits_z = self.get_z_bits(z_for_bit, self.bit_estimator_z, index)
        bits_z = self.get_z_bits_safe(z_for_bit, self.bit_estimator_z, index)
        if self.training:
