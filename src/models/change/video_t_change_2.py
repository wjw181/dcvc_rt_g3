...
class SpatialPrior_attention(nn.Module):
    def __init__(self, ):
        super().__init__()
        # Q、K、V 三路 conv，都接收 4*g_ch_y 输入，输出 2*g_ch_y
        ...
        # attention 缩放因子
        self.scale = (g_ch_y * 2) ** 0.5

        # 最后再加一层 DepthConvBlock + 1×1 Conv
    ...
    def forward(self, x):
        # x 通道数 = 4*g_ch_y (已在外部 concat 好)
        B, _, H, W = x.shape

        # Q/K/V
        Q = self.conv_q(x)   # B×Dk×H×W
        K = self.conv_k(x)   # B×Dk×H×W
        V = self.conv_v(x)   # B×Dv×H×W (Dv=Dk=2*g_ch_y)

        # flatten 到 B×C×N, N=H*W
        C = Q.size(1)
        N = H * W
        Qf = Q.view(B, C, N)
        Kf = K.view(B, C, N)
        ...
        # 点积注意力： (Kᵀ·Q)/√Dk -> B×N×N
        attn = torch.softmax(torch.bmm(Kf.transpose(1,2), Qf) / self.scale, dim=-1)

        # 注意力加权 V -> B×C×N
        Hf = torch.bmm(Vf, attn)

        # reshape 回 B×C×H×W
        H = Hf.view(B, C, H, W)

        # 最后再过一遍 DepthConvBlock + 1×1 Conv
        H = self.post(H)
...
class get_mask_all(nn.Module):
    def __init__(self, channel_m=None, channel_n=None):
        super(get_mask_all, self).__init__()
        self.mask_channel = nn.Sequential(
            nn.Conv2d(channel_m, 2*channel_n, 3, stride=1, padding=1),
            nn.LeakyReLU(),
            nn.Conv2d(2*channel_n, channel_n, 3, stride=1, padding=1),
        )
        self.mask = nn.Sequential(
            nn.Conv2d(channel_m+channel_n, channel_n, 3, stride=1, padding=1),
            nn.LeakyReLU(),
            nn.Conv2d(channel_n, channel_n, 3, stride=1, padding=1),
        )
        self.avg_pool = nn.AdaptiveAvgPool2d(1)

    def forward(self, x):
        x_channel_params = self.mask_channel(x)
        x_channel = torch.sigmoid(x_channel_params)
        mask_channel = self.avg_pool(x_channel)
        mask_channel_round = soft_round(mask_channel)
        mask_params = torch.cat((x_channel_params, x), dim=1)
        mask_0 = torch.sigmoid(self.mask(mask_params))
        mask_round = soft_round(mask_0)
        return x_channel_params, mask_channel_round, mask_round
...
class DMC(CompressionModel):
    def __init__(self):
        ...
        self.y_mask = get_mask_all(g_ch_y * 3, g_ch_y)
        ...
        self.prior_fusion = nn.Sequential(
            nn.Conv2d(g_ch_y * 4, g_ch_y * 5, 3, stride=1, padding=1),
            nn.LeakyReLU(0.2),
            nn.Conv2d(g_ch_y * 5, g_ch_y * 3, 3, stride=1, padding=1),
        )
        ...
        self.y_spatial_prior = SpatialPrior_attention()
    ...
    def forward(self, x, qp):
        ...
        z_hat= self.quant(z)
        params_1 = self.res_prior_param_decoder(z_hat, ctx_t)
        y_channel_params, y_mask_channel, y_mask = self.y_mask(params_1)
        params_2 = torch.cat((params_1, y_channel_params), dim=1)
        params = self.prior_fusion(params_2)
        y = y * y_mask_channel
        y_res,y_q,scales_hat, y_hat = \
            self.compress_prior_2x_t_1(y, params,y_mask, self.y_spatial_prior)
        ...
        pixel_num = H * W
        Y_mse = self.mse(x[:, 0:1, :, :], x_hat[:, 0:1, :, :])
        U_mse = self.mse(x[:, 1:2, :, :], x_hat[:, 1:2, :, :])
        V_mse = self.mse(x[:, 2:3, :, :], x_hat[:, 2:3, :, :])
        mse = 2 * Y_mse + 1*U_mse + 1*V_mse
        # mse = self.mse(x, x_hat)*1.5
        mse = torch.sum(mse, dim=(1, 2, 3)) / pixel_num

        ssim = self.ssim(x, x_hat)
        
        # print(y_for_bit.min(), y_for_bit.max())
        ...
        # print(f"bpp_y: {bpp_y}, bpp_z: {bpp_z}, mse: {mse}, ssim: {ssim}")
        # print(bpp_y, bpp_z, mse, ssim)
        result = {
            "bpp": bpp_y+bpp_z,
            "x_hat": x_hat,
            "mse": mse,
